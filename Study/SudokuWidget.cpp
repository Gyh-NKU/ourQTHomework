...
NumTableWidget::NumTableWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::NumTableWidget)
{
    ui->setupUi(this);
    ...
    for(int row=0; row<9; row++)
        ...
        {
            if(row/3 %2 == col/3%2)
                _btn[row][col].setStyleSheet("background-color: #ff9999");
            else
                _btn[row][col].setStyleSheet("background-color: #90baff");
            _btn[row][col].setFixedSize(55,55);
            ...
            connect(&_btn[row][col],&QPushButton::clicked,[this,row,col](){
               QPoint point = this->cursor().pos();//获取当前光标的位置
                SudokuForm* form = new SudokuForm(point,0,this,row,col);//创建一个小窗口
                form->exec();
           });
        }
...
NumTableWidget::~NumTableWidget()
{
    delete ui;
}

void NumTableWidget::process(bool show)
{
    ...
    randstart:
        ...
        for (r = 0; r < 81; r++)
            for (c = 0; c < 2; c++)
                backarr[r][c] = 0;
        i = r = c = rend = cend = back = layer = row = col = count = 0;
        //以上这些都是用来把所有的数据与变量清零，可以不用管它们
        ...
        else
        {
            ...
            while (count < i)
            {
                ...
                if (!flag[layer])
                {
                    ...
                    //_btn[row][col].setStyleSheet("background-color: #ffffff");
                    disconnect(&_btn[row][col],0,0,0);
        ...
        while (layer < 81) //因为要测试81次，layer代表81层的每一层
        {
            ...
            if (1 == flag[layer]) //这个分支用来测试是否计算到了固定数值，如果是，则跳过这一块
            {
                ...
                layer++; // 不然则当成前进式计算下一层
                continue;
                //QMessageBox::warning(0,tr("警告"),tr("改数字不可改动！"));
            }
...
void NumTableWidget::setNum(int row, int col, int num)//用num数字，设置为row行col列块的数字
{
    int value = num+1;
    for(int c=0; c<9; c++)
...
void NumTableWidget::clearNum(int row, int col) //擦除这一个块的数字
{
    _btn[row][col].setText("");
}

void NumTableWidget::on_showAnswerBtn_clicked()
{
    ui->showAnswerBtn->setEnabled(false);
    this->process(1);
}

void NumTableWidget::on_createEnigmaBtn_clicked()
{
    ui->createEnigmaBtn->setEnabled(false);
    this->process(0);
}

void NumTableWidget::on_clearAllBtn_clicked()
{
    for(int row=0; row<9; row++)
        for(int col=0; col<9; col++)
            _btn[row][col].setText("");
...
void NumTableWidget::closeEvent(QCloseEvent*){
    emit hasclosed();
}
